        ...
        for i in range(self.N):
            for j in range(self.N + self.M):
                for k in range(self.N + self.M):
                    self.mm[i,j,k] = (x[(i+1) % self.N] - x[(i-2) % self.N]) * (((i-1) % self.N == j) * ((self.N + 1) == k) + ((i-1) % self.N == k) * ((self.N + 1) == j))\
                                   + x[self.N + 1] * ((((i-1) % self.N) == j) * ((((i+1) % self.N) == k) - (((i-2) % self.N) == k)) + (((i-1) % self.N) == k) * ((((i+1) % self.N) == j) - (((i-2) % self.N) == j)))\
                                   + x[(i-1) % self.N] * (((((i+1) % self.N) == j) - (((i-2) % self.N) == j)) * ((self.N + 1) == k) + ((((i+1) % self.N) == k) - (((i-2) % self.N) == k)) * ((self.N + 1) == j))\
                                   + ((self.N + 2) == k) * (i == j) + ((self.N + 2) == j) * (i == k)
        gr = np.zeros(self.N + self.M)
        for i in range(self.N):
            gr[i] = xi.transpose() @ self.mm[i] @ la + xi[i]
