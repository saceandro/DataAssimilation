...
class Lorenz96:
    ...
    def gradient_adjoint(self, la, x, y):
        m = np.transpose(self.jacobian(x))
        gr = -m @ la - (x - y)
        return gr
    
    def adjoint(self, la, x, y, dt):
        m = np.transpose(self.jacobian(x))
        return dt * (m @ la) + (x - y) # dt * m?
...
class Adjoint:
    ...
    def gradient(self):
        ...
        la[self.steps-1]= self.x[self.steps-1] - self.y[self.steps-1]
        for i in range(self.steps-1, 0, -1):
            la[i-1] = self.dla(la[i], self.x[i-1], self.y[i-1], dt)
        return la[0]
...
scheme = Adjoint(lorenz.gradient, lorenz.adjoint, N, T, dt, x, y)
...
x_opt_bak = np.copy(x_opt)
gr_anal = scheme.gradient_from_x0(x_opt)
print (gr_anal)
gr_num = scheme.numerical_gradient_from_x0(x_opt_bak, 0.001)
print (gr_num)
...
for rep in range(1):
    ...
    x2 = np.zeros((steps, N))
    x2[0] = np.copy(res.x)
    scheme2 = Adjoint(lorenz.gradient, lorenz.gradient, N, T, dt, x2, y)
