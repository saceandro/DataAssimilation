...
#            print ((self.x[self.it*i] - self.y[i]) @ (self.x[self.it*i] - self.y[i]))
    ...
    #    cost = (xzero - xb) * (np.linalg.inv(B)) * (xzero - xb)
        for i in range(self.steps + 1):
            cost += (self.x[self.it*i][0:self.N] - self.y[i]) @ (self.x[self.it*i][0:self.N] - self.y[i]) / self.obs_variance
        return cost/2.0 # fixed
...
sysnoise_it = 100 # 20 40 50 100
sysnoise_steps = int(minute_steps/sysnoise_it)
sysnoise_variance = 0.11 # 0.04 0.06 0.07 0.11
obs_variance = 0.1
...
scheme = Adjoint(lorenz.gradient, lorenz.gradient_adjoint, N, T, dt, it, sysnoise_it, sysnoise_variance, obs_variance, x, obs, q)
...
scheme_true = Adjoint(lorenz.gradient, lorenz.gradient_adjoint, N, T, dt, sysnoise_it, it, sysnoise_variance, obs_variance, tob, obs, q2)
