    ...
    def tl(self, x):
        ...
        for i in range(self.N):
            for j in range(self.N):
                if (((i-1) % self.N) == j):
                    self.m[i][j] += (x[(i+1) % self.N] - x[(i-2) % self.N])
                if (((i+1) % self.N) == j):
                    self.m[i][j] += x[(i-1) % self.N]
                if (((i-2) % self.N) == j):
                    self.m[i][j] -= x[(i-1) % self.N]
                if ((i     % self.N) == j):
                    self.m[i][j] -= 1
        ...
        for i in range(self.N):
            for j in range(self.N + self.M):
                for k in range(self.N + self.M):
                    self.mm[i,j,k] = ((((i+1) % self.N) == j) - (((i-2) % self.N) == j)) * ((self.N + 1) == k) + ((((i+1) % self.N) == k) - (((i-2) % self.N) == k)) * ((self.N + 1) == j)
...
#            print("")
    ...
    def cbf(self, x0):
#        global axL, axR, axRR
        global count, trace, f_trace, cost_trace
        count += 1
        cos = self.cost(x0)
