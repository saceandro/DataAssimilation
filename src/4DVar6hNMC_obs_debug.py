...
window_day = 1.0
windows = int(day/window_day)
T_window = window_day * 0.2
minute_steps_window = int(T_window/dt)
steps_window = int(minute_steps_window/it)
...
t = np.arange(0.,0.2*windows,dt)

for ite in range(windows):    
    x_opt = np.loadtxt("data/assimilation_xzero.2.dat")
    beg = minute_steps_window * ite
    end = minute_steps_window * (ite+1)
    scheme = Adjoint(lorenz.gradient, lorenz.gradient_adjoint, N, T_window, dt, x[beg*it:end*it], obs[beg:end])
    scheme.cost(x_opt)
    print("x")
    plot_orbit(x[beg*it:end*it])
    compare_orbit(tob[beg*it:end*it], x[beg*it:end*it], 'true_orbit', 'initial value')
