...
class Adjoint:
    ...
    def orbit(self):
        ...
        for i in range(self.minute_steps-1):
            k1 = handler(self.dx, self.x[i]                , la_old[i])
            k2 = handler(self.dx, self.x[i] + k1*self.dt/2., la_old[i] + dla_old[i]*self.dt/2.)
            k3 = handler(self.dx, self.x[i] + k2*self.dt/2., la_old[i] + dla_old[i]*self.dt/2.)
            k4 = handler(self.dx, self.x[i] + k3*self.dt   , la_old[i] + dla_old[i]*self.dt)
        ...
        for i in range(self.steps-1, -1, -1):
            for j in range(it-1, -1, -1):
                ...
                if (n < self.it*self.steps - 1):
                    gr = dx_old[n]
                    k1 = handler(self.dla, la[n+1], x_old[n+1])
                    k2 = handler(self.dla, la[n+1] - k1*self.dt/2, x_old[n] + gr*self.dt/2)
                    k3 = handler(self.dla, la[n+1] - k2*self.dt/2, x_old[n] + gr*self.dt/2)
                    k4 = handler(self.dla, la[n+1] - k3*self.dt, x_old[n])
                    la_gr = (k1 + 2*k2 + 2*k3 + k4)/6.
                    la[n] = la[n+1] + la_gr * self.dt
                    dla_old[n] = la_gr
            la[self.it*i] += x_old[self.it*i] - self.y[i]
        la_old = np.copy(la)
        return la[0]
...
la_old = np.zeros((minute_steps, N))
dla_old = np.zeros((minute_steps, N))
x_old = np.zeros((minute_steps, N))
dx_old = np.zeros((minute_steps, N))
