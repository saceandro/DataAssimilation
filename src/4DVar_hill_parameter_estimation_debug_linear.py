...
class Linear:
    ...
    def gradient(self, t, x):
        d = np.zeros(self.N)
        d[0] = (1.0 + 0.1*math.cos(t)) * x[1] + math.cos(t)
        d[1] = (-1.0 + 0.1*math.cos(t)) * x[0] + math.cos(t)
        return d
    
    def gradient_adjoint(self, la, t, x):
        mt = np.zeros((self.N,self.N))
        mt[0][1] = -1.0 + 0.1*math.cos(t)
        mt[1][0] = 1.0 + 0.1*math.cos(t)
        gr = mt @ la
        return gr
...
#%%
def plot_orbit(dat, lab):
    fig = plt.figure()
    plt.plot(dat[:,0],dat[:,1], label=lab)
    plt.show()
...
def compare_orbit3(dat1, dat2, dat3, lab1, lab2, lab3):
    fig = plt.figure()
    plt.plot(dat1[:,0],dat1[:,1],label=lab1)
    plt.plot(dat2[:,0],dat2[:,1],label=lab2)
    plt.plot(dat3[:,0],dat3[:,1],label=lab3)
    plt.legend()
    plt.show()
...
ctrl = ramp
pref = "/Users/konta/bitbucket/androsace/dacamp/task1/oscillator_data/"
...
tob = np.loadtxt(pref + "year.1.dat", ndmin=2)
tob2 = np.loadtxt(pref + "year.2.dat", ndmin=2)
...
compare_orbit3(tob[0:minute_steps], obs[0:steps], scheme.x[0:minute_steps], 'true_orbit', 'observed', 'initial value')
compare_orbit(tob[0:minute_steps], scheme.x[0:minute_steps], 'true_orbit', 'assimilated')
...
compare_orbit3(tob[0:minute_steps], scheme.x[0:minute_steps], obs[0:steps], 'true orbit', 'assimilated', 'observed')
