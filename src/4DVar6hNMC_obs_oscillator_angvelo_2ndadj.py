...
class Linear:
    ...
    def gradient_secondadj(self, nu, x, la, xi):
        ...
        gr[0] =  xi[1]*la[2] + xi[2]*la[1] + xi[0]
        gr[1] = -xi[0]*la[2] - xi[2]*la[0] + xi[1]
        return self.gradient_tl_T(nu, x) + gr
...
#            print("")
    ...
    def cbf(self, x0):
        global count, trace, cost_trace, omega_trace
        count += 1
        cos = self.cost(x0)
        cost_trace.append(cos)
        omega_trace.append(x0[self.N])
        for i in range(self.N):
            trace[i].append(x0[i])
...
tob = np.loadtxt(pref + "nosysnoise.1.dat")
tob2 = np.loadtxt(pref + "nosysnoise.2.dat")
...
true_param = np.array([1.])

fig = plt.figure()
plt.plot([true_param[0] for i in range(len(omega_trace))], 'r')
plt.plot(omega_trace, 'b')
plt.errorbar(len(omega_trace)-1, omega_trace[-1], yerr=hessian_inv[N,N], fmt='b')
plt.legend()
plt.xlabel('minimizer iteration')
