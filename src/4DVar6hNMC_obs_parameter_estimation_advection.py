...
class Lorenz96:
    ...
    def gradient(self,x):
        ...
        d[0]        = x[self.N] * (x[1]   - x[self.N-2]) * x[self.N-1] - x[0]        + self.F
        d[1]        = x[self.N] * (x[2]   - x[self.N-1]) * x[0]        - x[1]        + self.F
        for i in range(2, self.N-1):
            d[i]    = x[self.N] * (x[i+1] - x[i-2])      * x[i-1]      - x[i]        + self.F
        d[self.N-1] = x[self.N] * (x[0]   - x[self.N-3]) * x[self.N-2] - x[self.N-1] + self.F
        return d
    ...
    def gradient_adjoint(self, la, x):
        ...
        for i in range(self.N):
            for j in range(self.N):
                if (((i-1) % self.N) == j):
                    mt[j][i] += x[self.N] * (x[(i+1) % self.N] - x[(i-2) % self.N])
                if (((i+1) % self.N) == j):
                    mt[j][i] += x[self.N] * x[(i-1) % self.N]
                if (((i-2) % self.N) == j):
                    mt[j][i] -= x[self.N] * x[(i-1) % self.N]
                if ((i     % self.N) == j):
                    mt[j][i] -= 1
            mt[N][i] = (x[(i+1) % self.N] - x[(i-2) % self.N]) * x[(i-1) % self.N]
        gr = mt @ la
        return gr
...
x_opt[N] = 0.5 # initial guess for a
